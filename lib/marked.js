...
inline.widget = replace(inline.widget)
  ('href', inline._href)
  ();
...
InlineLexer.prototype.output = function(src) {
  ...
  while (src) {
    ...
    if (cap = this.rules.widget.exec(src)) {
      src = src.substring(cap[0].length);
      out += this.renderer.widget(cap[2], cap[3]);
      continue;
    }
...
InlineLexer.prototype.outputLink = function(cap, link) {
  ...
  if (flag === '!') {
    out = this.renderer.image(href, title, escape(cap[1]));
  } else if (this.options.widgets && flag === '$') {
    out = this.renderer.widget(href, title, escape(cap[1]));
  } else {
    if (flag === '$') out += '$';
    out += this.renderer.link(href, title, this.output(cap[1]));
  }
...
Renderer.prototype.widget = function(href, title) {
  var out = '[[' + href;
  if (title)
    out += ' "' + title + '"';
  out += ']]';
